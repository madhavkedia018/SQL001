...
BEGIN
    ...
    WHERE d.CurrentFlag = 1 AND ( d.Name <> s.Name OR d.Address <> s.Address OR d.City <> s.City )

    INSERT INTO DimCustomer (CustomerID, Name, Address, PreviousAddress, City, StartDate, EndDate, CurrentFlag)
    SELECT s.CustomerID, s.Name, s.Address, d.Address, s.City, @Today, NULL, 1
    FROM StgCustomer s
    JOIN DimCustomer d ON s.CustomerID = d.CustomerID
    WHERE d.CurrentFlag = 0 AND d.EndDate = @Today
